...
class _OfferRideScreenState extends State<OfferRideScreen> {
  ...
  bool _isOffering = false; // shows buffer/spinner
  ...
  Future<void> _onOffer() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('https://mujdriveride.shivamrajdubey.tech/rides/offer-ride'),
        headers: {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          IconButton(icon: const Icon(Icons.remove_circle_outline), onPressed: () => onDelta(-1)),
          Text('$value', style: const TextStyle(fontSize: 16)),
          IconButton(icon: const Icon(Icons.add_circle_outline), onPressed: () => onDelta(1)),
        ],
      ),
    );
  }
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                                  ...
                                  children: [
                                    IconButton(
                                        ...
                                        onPressed: () => setState(() => _availableSeats =
                                            (_availableSeats - 1).clamp(0, _totalSeats))),
                          ...
                          if (_smallSelected)
                            _buildStepper('Small Bags', _smallCount,
                                (d) => setState(() => _smallCount = (_smallCount + d).clamp(0, 15))),
                          if (_mediumSelected)
                            _buildStepper('Medium Bags', _mediumCount,
                                (d) => setState(() => _mediumCount = (_mediumCount + d).clamp(0, 15))),
                          if (_largeSelected)
                            _buildStepper('Large Bags', _largeCount,
                                (d) => setState(() => _largeCount = (_largeCount + d).clamp(0, 15))),
                          ...
                          ElevatedButton(
                            ...
                            onPressed: _isOffering ? null : _onOffer,
                            child: _isOffering
                                ? const SizedBox(
                                    width: 24,
                                    height: 24,
